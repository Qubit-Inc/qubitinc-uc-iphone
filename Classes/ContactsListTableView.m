...
- (void) loadSIpFilteredData {
    LOGI(@"Search contact list on SIP address");
    @synchronized(addressBookMap) {
        //Set all contacts from ContactCell to nil
        for (NSInteger j = 0; j < [self.tableView numberOfSections]; ++j)
        {
        ...
        for (NSString *addr in sortedAddresses) {
            @synchronized(
                ...
                if (add && name != nil) {
                    ...
                    [contact.sipAddresses filteredArrayUsingPredicate:predicate];
                    NSArray *filteredarr = [NSMutableArray arrayWithArray:[contact.sipAddresses filteredArrayUsingPredicate:predicate]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString* imageName = [cell.avatarImage accessibilityIdentifier];
    if([imageName containsString:@"avatar"] || imageName == nil ) {
        NSRange myRange = NSMakeRange(0, 1);
        [cell.avatarImage setImage: [cell.avatarImage.image drawWatermarkText:[cell.nameLabel.text substringWithRange:myRange]]];
    }
    return cell;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UILabel *bottomSeperatoLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 39, tableView.frame.size.width, 1)];
    [bottomSeperatoLabel setBackgroundColor:[UIColor lightGrayColor]];
    
    [tempView addSubview:tempLabel];
    [tempView addSubview:bottomSeperatoLabel];
