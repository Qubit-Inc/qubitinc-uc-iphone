...
- (void)setRecording:(NSString *)arecording {
    ...
    if(_recording) {
        NSArray *parsedRecording;
        if ([_recording containsString:@"voicemail_"])
            parsedRecording = [LinphoneUtils parseVoiceMailName:_recording];
        else
            parsedRecording = [LinphoneUtils parseRecordingName:_recording];
...
- (void)onShareButtonPressed {
    ...
    [activityVC setCompletionWithItemsHandler:^(UIActivityType __nullable activityType, BOOL completed, NSArray * __nullable returnedItems, NSError * __nullable activityError) {
        ...
        if([file containsString:@"recording_"])
