...
- (void)viewWillAppear:(BOOL)animated {
	...
	[_sideMenuEntries
		addObject:[[SideMenuEntry alloc] initWithTitle:NSLocalizedString(@"Account", nil)
											  ...
											  tapBlock:^()
    {
        ...
        if(accounts == NULL){
            ...
            NSString *valueToSave = @"Account";
            [[NSUserDefaults standardUserDefaults] setObject:valueToSave forKey:@"SettingType"];
    ...
    [_sideMenuEntries
        addObject:[[SideMenuEntry alloc] initWithTitle:NSLocalizedString(@"UC Features", nil)
                                                 image:[UIImage imageNamed:@"voicemail.png"]
                                              tapBlock:^() {
        NSString *valueToSave = @"UCFeatures";
        [[NSUserDefaults standardUserDefaults] setObject:valueToSave forKey:@"SettingType"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    ...
    [_sideMenuEntries
        addObject:[[SideMenuEntry alloc] initWithTitle:NSLocalizedString(@"Voice Mail", nil)
                                                 image:[UIImage imageNamed:@"voicemail.png"]
                                              tapBlock:^() {
    ...
    [_sideMenuEntries
        addObject:[[SideMenuEntry alloc] initWithTitle:NSLocalizedString(@"FAX", nil)
                                                 image:[UIImage imageNamed:@"voicemail.png"]
                                              tapBlock:^() {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0 && [LinphoneManager isLcInitialized]) {
        ...
        SideMenuCell* sideMenuCell = (SideMenuCell *) [tableView dequeueReusableCellWithIdentifier:@"SideMenuCell"];
		SideMenuEntry *entry = [_sideMenuEntries objectAtIndex:indexPath.row];
        sideMenuCell.labelMenuTitle.text = entry->title;
        [sideMenuCell setUpCell:indexPath.row];
        return  sideMenuCell;
	}
	return cell;
}
