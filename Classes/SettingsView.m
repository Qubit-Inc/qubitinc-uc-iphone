...
- (void)toggledValue:(id)sender {
    ...
    if ([toggle isOn]) {
        [self updateUcFeatureValue:@"true" forKey:[toggle key]];
        ...
        [self updateUcFeatureValue:@"false" forKey:[toggle key]];
...
-(void)updateUcFeatureValue:(NSString*) value forKey:(NSString*)key {
    LinphoneAppDelegate* del = [LinphoneAppDelegate appDelegate];
    if(del.isUCFeatureLoading) {
        NSLog(@"IsLoading");
        return;
    }
    ...
    if (default_proxy != NULL) {
        ...
        NSLog(@"%@", key);
        NSString *postParamKey;
        if([key isEqualToString:@"dial_out_isd"]) {
            postParamKey = @"dialisd";
        }
        else if([key isEqualToString:@"dial_out_intercom"]) {
            postParamKey = @"dialout";
        }
        else if([key isEqualToString:@"dial_out_std"]) {
            postParamKey = @"dialstd";
        }
        else if([key isEqualToString:@"dial_out_local"]) {
            postParamKey = @"diallocal";
        }
        ...
        [param setValue:encryptedUserName forKey:@"contact"];
        [param setValue:[del getEncryptedName:postParamKey] forKey:@"featurecode"];
        [param setValue:encryptedValue forKey:@"featurecodedata"];
        //[param setValue:encryptedUserName forKey:[del getEncryptedName:@"contact"]];
        //[param setValue:encryptedValue forKey:[del getEncryptedName:postParamKey]];
        
        [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeClear];
        [SVProgressHUD show];
        
        HTTPClient *sharedManager = [HTTPClient sharedManager];
        [sharedManager callPostApi:@"updatefeature" parameters:param andCompletion:^(id  _Nonnull response, NSError * _Nonnull error) {
            [SVProgressHUD dismiss];
            if(error == nil) {
                NSLog(@"Response:-- %@", response);
            }
            else {
                NSLog(@"Failed to feature:-- %@", error);
            }
        }];
    }
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _appDelegate = [LinphoneAppDelegate appDelegate];
    _appDelegate.isUCFeatureLoading = YES;
...
- (void)settingsViewControllerWillAppear:(IASKAppSettingsViewController *)sender {
    isRoot = (sender.file == nil || [sender.file isEqualToString:@"UCFeatures"] || [sender.file isEqualToString:@"Settings"] || [sender.file isEqualToString:@"Account"]);
    // _titleLabel.text = sender.title;
    NSString *settingType = [[NSUserDefaults standardUserDefaults]
        stringForKey:@"SettingType"];
    if([settingType  isEqual: @"UCFeatures"]) {
        _titleLabel.text = @"UC Features";
        //Get UC Features Data
        [self getUCFeaturesData];
    }
    else if([settingType  isEqual: @"Settings"]) {
    ...
    if ([sender.file isEqualToString:@"Account"]) {
        ...
        [settingsStore transformAccountToKeys:nil];
...
-(void)getUCFeaturesData {
    HTTPClient *sharedManager = [HTTPClient sharedManager];
    NSDictionary *param = [[NSDictionary alloc] init];
    LinphoneProxyConfig *default_proxy = linphone_core_get_default_proxy_config(LC);
    
    if (default_proxy != NULL) {
        [[_settingsController tableView] setHidden:TRUE];
        const char *userName = linphone_address_get_username(linphone_proxy_config_get_identity_address(default_proxy));
        LinphoneAppDelegate *del = [LinphoneAppDelegate appDelegate];
        NSString *encryptedUserName = [del getEncryptedName:[NSString stringWithFormat:@"%s", userName]];
        NSString *path = [NSString stringWithFormat:@"getucfeaturelist/%@", encryptedUserName];
        [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeClear];
        [SVProgressHUD show];
        ...
        [sharedManager callGetApi:path parameters:param :^(id  _Nonnull response, NSError * _Nonnull error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for(NSDictionary *json in arrayData) {
                    ...
                    NSString* servicecode = [json valueForKey:@"servicecode"];
                    
                    if([servicecode isEqualToString:@"dialing-isd"]) {
                        [self->settingsStore setBool:YES forKey:@"dial_out_isd"];
                    }
                    else if([servicecode isEqualToString:@"dialing-intercom"]) {
                        [self->settingsStore setBool:YES forKey:@"dial_out_intercom"];
                    }
                    else if([servicecode isEqualToString:@"dialing-std"]) {
                        [self->settingsStore setBool:YES forKey:@"dial_out_std"];
                    }
                    else if([servicecode isEqualToString:@"dialing-local"]) {
                        [self->settingsStore setBool:YES forKey:@"dial_out_local"];
                    }
                    else if([servicecode isEqualToString:@"dnd"]) {
                        [self->settingsStore setBool:YES forKey:@"dnd"];
                    }
                    else if([servicecode isEqualToString:@"follow-me"]) {
                        [self->settingsStore setObject:[json valueForKey:@"data"] forKey:@"follow_me"];
                    }
                    else if([servicecode isEqualToString:@"fwd-all"]) {
                        [self->settingsStore setObject:[json valueForKey:@"data"] forKey:@"call_forward"];
                    }
                    else if([servicecode isEqualToString:@"fwd-no-ans"]) {
                       //Handle this case
                    }
                    else if([servicecode isEqualToString:@"VM"]) {
                       //Handle this case
                    }
                    else if([servicecode isEqualToString:@"fwd-busy"]) {
                       //Handle this case
                    }
                }
                ...
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                    _appDelegate.isUCFeatureLoading = NO;
                });
                [[_settingsController tableView] setHidden:FALSE];
                [[_settingsController tableView] reloadData];
            });
            if (error == nil){
                NSDictionary *jsonDictionary=(NSDictionary *)response;
