...
- (UIView *)tableView:(UITableView*)tableView viewForHeaderInSection:(NSInteger)section {
	if ([self.delegate respondsToSelector:@selector(settingsViewController:tableView:viewForHeaderForSection:)]) {
        ...
        UIView* underLineLabel = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 39.0f, self.view.frame.size.width, 1.0f)];
        underLineLabel.backgroundColor = CORAL_THEME_COLOR;
        
        UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(20.0f, 07.0f, self.view.frame.size.width-40, 30.0f)];
        NSString *header = [self.settingsReader titleForSection:section];
        label.textColor = CORAL_THEME_COLOR;
        ...
        if (@available(iOS 12.0, *)) {
            if( self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark ){
                ...
                UIColor *bgColor = [UIColor colorWithRed:43 / 255.0f green:43 / 255.0f blue:45 / 255.0f alpha:1.0];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    IASKTextField *currentTextField = (IASKTextField*) textField;
    if([currentTextField.key isEqualToString:@"follow_me"] || [currentTextField.key isEqualToString:@"call_forward"]){
        [self updateUcFeatureValue:currentTextField.text forKey:currentTextField.key];
    }
	return YES;
}
...
-(void)updateUcFeatureValue:(NSString*) value forKey:(NSString*)key {
    ...
    if (default_proxy != NULL) {
        ...
        if([key isEqualToString:@"follow_me"]) {
