...
@objc class AppManager: NSObject {
    ...
    @objc static func voicemailFilePath(otherparty: String, createdEpoc:String) -> String {
        var filePath = "voicemail_"
        filePath = filePath.appending(otherparty.isEmpty ? "unknow" : otherparty)
        let epocDate = Date.init(timeIntervalSince1970: TimeInterval.init(Int(createdEpoc) ?? 0))
        let dateFormat = DateFormatter()
        dateFormat.dateFormat = "E-d-MMM-yyyy-HH-mm-ss"
        let date = dateFormat.string(from: epocDate)
        ...
        Log.directLog(BCTBX_LOG_MESSAGE, text: "file path is \(writablePath)")
        return writablePath
        //file name is voicemail_contact-name_dayName-day-monthName-year-hour-minutes-seconds
        //The recording prefix is used to identify recordings in the cache directory.
        //We will use name_dayName-day-monthName-year to separate recordings by days, then hour-minutes-seconds to order them in each day.
    }
}
