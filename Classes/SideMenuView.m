...
- (void)viewDidLoad {
    ...
    [_sideMenuTableViewController.tableView registerNib:nib forCellReuseIdentifier:@"SideMenuCell"];
    
    
    [_shadowView.layer setShadowColor:[UIColor grayColor].CGColor];
    [_shadowView.layer setShadowOpacity:0.6];
    [_shadowView.layer setShadowRadius:5.0];
    [_shadowView.layer setShadowOffset:CGSizeMake(8.0, 5.0)];
...
//	}
    ...
    [ContactDisplay setDisplayNameLabel:_nameLabel forAddress:addr];
            ...
            NSString *sipIP =  addr? [NSString stringWithUTF8String:linphone_address_as_string(addr)] : NSLocalizedString(@"No address", nil);
            NSRange pos = [sipIP rangeOfString:@"@"];
            if (pos.location != NSNotFound) {
            NSString *prefix = [sipIP substringFromIndex:pos.location];
            _addressLabel.text = [prefix substringFromIndex:1];
            }
